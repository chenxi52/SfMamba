...
def triton_cross_scan_flex(
    ...
    NH: tl.constexpr,
    NW: tl.constexpr,
    mid_w_block: tl.constexpr
):
    ### N 是行/列块数，B是每块大小，D是输入的大小
    # x_layout = 0
    ...
    i_h, i_w = (i_hw // NW), (i_hw % NW) ## i_h, i_w 是块的索引坐标，（0-1），例如n=5, i_w索引是相对每块的
    ...
    pos_h = (i_h * BH + tl.arange(0, BH)[:, None]) #i_h * BH 计算当前块在高度维度上的起始位置， tl.arange(0, BH)[:, None] 生成一个列向量，表示块内的相对位置。
    pos_w = (i_w * BW + tl.arange(0, BW)[None, :])
    neg_h = (DH - i_h * BH - 1 - tl.arange(0, BH)[:, None]) ### 二维数组了？
    neg_w = (DW - i_w * BW - 1 - tl.arange(0, BW)[None, :])
    if scans == 0:
        # none; trans; flip; trans + flip;
        HWRoute0 = pos_h * DW + pos_w # 这是线性索引
        ...
        # none; none; flip; flip;
        HWRoute0 = pos_h * DW + pos_w ##将高度位置乘以宽度总大小再加上宽度位置得到的
        HWRoute1 = HWRoute0
        HWRoute2 = neg_h * DW + neg_w # flip
        HWRoute3 = HWRoute2      
    elif scans == 3:
        ## 只考虑一维的话，h=0，
        assert DH == 1, "scans=3模式仅在高度为1时有效"
        # 生成调整后的块顺序：[中间块, 中间+1, ..., 最后, 0, 1, ..., 中间-1]
        range_w1 = tl.arange(mid_w_block, NW)  # 中间及右侧块
        range_w2 = tl.arange(0, mid_w_block)             # 左侧块
        combined_w_blocks = tl.cat([range_w1, range_w2])
        
        # 获取当前块调整后的索引
        adjusted_w_block = combined_w_blocks[i_w]  # i_w来自i_hw % NW
        
        # 计算调整后的宽度位置
        posm_w = adjusted_w_block * BW + tl.arange(0, BW)[None, :]
        negm_w = DW - adjusted_w_block * BW - 1 - tl.arange(0, BW)[None, :]
        
        # 强制高度位置为0（因H=1）
        posm_h = tl.zeros([BH, 1], dtype=tl.int32)
        negm_h = tl.zeros([BH, 1], dtype=tl.int32)
        HWRoute0 = posm_h * DW + posm_w    # 中间起点正向扫描
        HWRoute1 = pos_h * DW + pos_w  
        HWRoute2 = negm_h * DW + negm_w    # 中间起点逆向扫描
        HWRoute3 = neg_h * DW + neg_w   
...
class CrossScanTritonF(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, in_channel_first=True, out_channel_first=True, one_by_one=False, scans=0):
        ...
        # 计算宽度中间位置（物理位置，非块索引）
        mid_w_pos = W // 2 + (W % 2)  # 处理奇数长度，向右偏
        # 将整个宽度划分为块后，找到中间块
        mid_w_block = mid_w_pos // BW  # 中间位置所在的块索引
        ...
        triton_cross_scan_flex[(NH * NW, NC, B)](
            ...
            (0 if out_channel_first else 1), 0, 
            (0 if not one_by_one else 1), scans, 
            BC, BH, BW, C,H, W, NH, NW, mid_w_block
        )
        return y
