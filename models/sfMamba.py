...
from fvcore.nn.print_model_statistics import flop_count_table
...
class mamba_init:
    ...
    def A_log_init(d_state, d_inner, copies=-1, device=None, merge=True):
        ...
        # merge 参数为 True, A_log: (copies * d_inner, d_state) 的张量
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        FORWARD_TYPES = dict(
            ...
            v052dm=partial(self.forward_corev2, force_fp32=False, no_einsum=True, scan_mode='bimidi'), # debug
            # ===============================
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        _scan_mode = dict(cross2d=0, unidi=1, bidi=2, cascade2d=-1, bimidi=3).get(scan_mode, None) if isinstance(scan_mode, str) else scan_mode # for debug
...
class VSSM_UDA(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.classifier = nn.Sequential(OrderedDict(
            ...
            avgpool=nn.AdaptiveAvgPool2d(1), ## 对每个channel进行avgpool
    ...
    def save_init_weights(self, save_name, freeze_name):
        self.init_weights = {}
        for name, param in self.named_parameters():
            for sn in save_name:
                if sn in name:
                    self.init_weights[name] = param.data.detach().clone()
        for name, param in self.named_parameters():
            for fn in freeze_name:
                if fn not in name:
                    self.init_weights[name] = param.data.detach().clone()
                    
    def return_reg_loss(self):
        reg_loss = 0    
        for name, param in self.named_parameters():
            if name in self.init_weights:
                reg_loss += torch.sum((param - self.init_weights[name]) ** 2)
        return  reg_loss
    ...
    def flops(self, shape=(3, 224, 224), verbose=True):
        ...
        supported_ops={
            ...
            "prim::PythonOp.SelectiveScanCuda": partial(selective_scan_flop_jit, backend=None, verbose=verbose),
        }
        ...
        flops_table = flop_count_table(
            flops = FlopCountAnalysis(model, (input,)).set_op_handle(**supported_ops),
            max_depth=100,
            activations=None,
            show_param_shapes=True,
        )
        del model, input
        return sum(Gflops.values()) ,flops_table
...
class Ch_VSS(nn.Module):
    def __init__(
        ...
        forward_type_1d = kwargs.get("d1_forward_type", "v052d")
        self.residual = kwargs.get("neck_res", False)
        self.ss1d = SS2D(self.input_res**2, d_state, ssm_ratio, dt_rank, 
                         act_layer, d_conv, conv_bias, dropout, bias, 
                         dt_min, dt_max, dt_init, dt_scale, 
                         dt_init_floor, initialize, forward_type_1d, 
                         channel_first, **kwargs)
    
    def forward(self, x: torch.Tensor, **kwargs):
        B, C, H, W = x.shape
        if self.residual:
            x_prepaired = x
        x_mix = x.view(B, -1, self.input_res**2)
        ...
        x_mix = x_mix.view(B,C,H,W)
        if self.residual:
            x_mix = x_mix + x_prepaired 
        return x_mix
    

class SfMamba(VSSM_UDA):
    def __init__(
        self, 
        patch_size=4, 
        in_chans=3, 
        ...
        **kwargs,
        ...
        norm_layer: nn.Module = _NORMLAYERS.get(norm_layer.lower(), None)
        neck_norm_layer: nn.Module = _NORMLAYERS.get(neck_norm_layer.lower(), None)
        ssm_act_layer: nn.Module = _ACTLAYERS.get(ssm_act_layer.lower(), None)
        mlp_act_layer: nn.Module = _ACTLAYERS.get(mlp_act_layer.lower(), None)
        norm_layer2d: nn.Module = _NORMLAYERS.get('BN'.lower(), None)
        ...
        self.classifier.add_module('head', nn.Linear(self.num_features, self.num_classes))
        self.fusion_blocks = nn.ModuleList()
        for i_layer in range(self.num_layers):
            if i_layer != self.num_layers-1: continue
            if fusion_module== 'ssm':
                self.fusion_blocks.append(self._make_layer(
                    dim = self.dims[i_layer],
                    drop_path = 0.,
                    depth=neck_deep,
                    use_checkpoint=use_checkpoint,
                    norm_layer=neck_norm_layer,
                    channel_first=self.channel_first,
                    # =================
                    ssm_d_state=ssm_d_state,
                    ssm_ratio=ssm_ratio,
                    ...
                    # =================
                    _SS2D=Ch_VSS,
                    input_res=input_res[i_layer],
                    **kwargs,
                ))
            self.fusion_blocks.append(norm_layer2d(self.num_features))
            
        self.apply(self._init_weights)
        self.bg_ratio = bg_ratio
        self.relu_bef_cls = True if 'relu' in neck else False
        self.relu = nn.ReLU(inplace=True)


    def forward_layers(self, x, back_layer=-1): 
        ### back_layer = 0 1 2 3
        back_feat = None
        for i, layer in enumerate(self.layers):
            ...
            if i!=self.num_layers-1: 
                ...
                continue
            for fu_i in self.fusion_blocks:
                x = fu_i(x)
            if layer.downsample is not None:
                x = layer.downsample(x)
        if self.relu_bef_cls:
            x = self.relu(x)
        return x, back_feat
    

    def forward_shuffle(self, x, y=None, return_feat=False):
        patches = self.patch_embed(x)
        assert self.pos_embed is None
        input_shape = patches.shape

        fmap, _ = self.forward_layers(patches)
        all_pre = self.classifier(fmap)
        assert y is not None

        all_o = F.one_hot(y.long(), self.num_classes)
        all_g = autograd.grad((all_pre * all_o).sum(), fmap)[0]
        all_g = torch.mean(all_g, dim=(2,3), keepdim=True) # B, D, 1,1
        all_g = torch.sum(all_g * fmap, dim=1)
        all_g = TF.resize(all_g, input_shape[2:4]) 
        all_g_flatten = all_g.flatten(1, 2) 

        _, indices = torch.sort(all_g_flatten.detach().cpu(), dim=-1)
        fea_len = input_shape[2] * input_shape[3]
        percentile = int(fea_len * self.bg_ratio)

        patches_reshaped = patches.permute(0,2,3,1).view(input_shape[0], -1, input_shape[1])
        fore_indices = indices[:, percentile:] 
        bg_indices = indices[:, :percentile]

        fore_patches = patches_reshaped[torch.arange(input_shape[0]).unsqueeze(1), fore_indices]
        bg_patches = patches_reshaped[torch.arange(input_shape[0]).unsqueeze(1), bg_indices]
        bg_order = torch.randperm(percentile)
        bg_patches = bg_patches[:, bg_order]

        result = torch.empty_like(patches.permute(0,2,3,1).flatten(1, 2))
        result[torch.arange(input_shape[0]).unsqueeze(1),fore_indices] = fore_patches
        result[torch.arange(input_shape[0]).unsqueeze(1),bg_indices] = bg_patches
        result = result.view(-1, input_shape[2], input_shape[3], input_shape[1]).permute(0,3,1,2)
        
        all_patches = torch.cat((patches, result))
        all_features,_ = self.forward_layers(all_patches)
        all_pre = self.classifier(all_features)
        if return_feat:
            return all_pre, x
        return all_pre
    ...
    def forward(self, x, y=None, return_feat = False, 
                scs=False, **kwargs):
        if scs:
            return self.forward_shuffle(x, y, return_feat=return_feat, **kwargs)
        else:
            x = self.patch_embed(x)
            if self.pos_embed is not None:
                pos_embed = self.pos_embed.permute(0, 2, 3, 1) if not self.channel_first else self.pos_embed
