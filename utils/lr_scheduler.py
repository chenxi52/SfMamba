...
def build_scheduler(config, optimizer, n_iter_per_epoch, warm_epochs=-1):
    ...
    elif config.TRAIN.LR_SCHEDULER.NAME=='expDic':
...
class MultiStepLRScheduler(Scheduler):
    def __init__(self, optimizer: torch.optim.Optimizer, gamma=0.1, warmup_t=0, warmup_lr_init=0, t_in_epochs=True) -> None:
...
class CustomLRScheduler(Scheduler):
    def __init__(self, optimizer: torch.optim.Optimizer, gamma=-0.75, warmup_t=0, warmup_lr_init=0, t_in_epochs=True,t_all=None) -> None:
        super().__init__(optimizer, param_group_field="lr")
        self.gamma = gamma
        self.warmup_t = warmup_t
        self.warmup_lr_init = warmup_lr_init
    ...
    def _get_lr(self, t):
        if t < self.warmup_t:
            ...
            lrs = [v * ((1+10 * (t-self.warmup_t)/self.t_all) ** self.gamma) for v in self.base_values]
        return lrs
